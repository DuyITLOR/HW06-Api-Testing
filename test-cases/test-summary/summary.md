...
- Sinh lúc: `2026-08-22T15:07:06.324Z`

| API | Lượt chạy (UTC) | Iteration | Request | Assertion | Passed | **Failed** | Thời lượng |
|---|---|---|---|---|---|---|---|
| API-01 · Pool A · GET /api/products | 2026-08-22T15:00:11.332Z | 1 | 62 | 155 | 126 | **29** | 1.1s |
| API-02 · Pool B · POST /api/cart | 2026-08-22T15:00:13.645Z | 1 | 52 | 85 | 55 | **30** | 1.0s |
| API-03 · Pool C · PUT /api/products/:id | 2026-08-22T15:00:16.102Z | 1 | 57 | 93 | 63 | **30** | 1.1s |
| **Tổng** | | | **171** | **333** | **244** | **89** | **3.2s** |
...
Raw: `reports/newman/23127178_api-01-products-search_ci-32580345226.json`
...
Raw: `reports/newman/23127178_api-02-cart-add_ci-32580345226.json`
...
| TC-CART-029 · bước 6: sau checkout **giỏ phải rỗng** | trả về đúng 0 dòng | expected 32 to deeply equal +0 |
| TC-CART-038 · GET /api/cart đúng schema giỏ hàng | giỏ hàng đúng schema (spec §4.2) | expected data to satisfy schema but found following errors:  data[1].id should be integer, data[2].id should be integer, data[3].id should be integer, data[4].i |
| TC-CART-101 · **price tampering**: gửi giá 1 đồng cho sản phẩm 111.000 | status thuộc [400,422] | expected [ 400, 422 ] to include 200 |
| TC-CART-102 · **hệ quả** của price tampering: giỏ không được chứa giá 1 đồng | không có dòng nào bị sửa giá | có 5 dòng giá sai: [{"id":"24","name":"HW06-Cart-Fixture","price":0,"quantity":1},{"id":"24","name":"HW06-Cart-Fixture","price":-1000,"quantity":1},{"id":"24"," |
...
| TC-CART-104 · **hệ quả** của mass assignment: giỏ không được chứa field lạ | không dòng nào có field lạ role/isAdmin | có 2 dòng mang field lạ: expected 2 to deeply equal +0 |
| TC-CART-106 · bước 2: thêm sản phẩm **vừa bị xoá** vào giỏ | status thuộc [400,404] | expected [ 400, 404 ] to include 200 |
| TC-CART-107 · **bất biến trạng thái giỏ** sau toàn bộ input sai ở trên | không dòng nào có quantity <= 0 | có 6 dòng quantity không hợp lệ: [{"id":"24","name":"HW06-Cart-Fixture","price":111000,"quantity":0},{"id":"24","name":"HW06-Cart-Fixture","price":111000,"quant |
| TC-CART-107 · **bất biến trạng thái giỏ** sau toàn bộ input sai ở trên | không dòng nào có name rỗng hoặc thiếu (hệ quả TC-021) | có 2 dòng không có tên: [{},{"id":"24","name":"","price":111000,"quantity":1}]: expected 2 to deeply equal +0 |
| TC-CART-107 · **bất biến trạng thái giỏ** sau toàn bộ input sai ở trên | không dòng nào thiếu price (hệ quả TC-020) | có 2 dòng không có giá: [{},{"id":"24","name":"HW06-Cart-Fixture","quantity":1}]: expected 2 to deeply equal +0 |

### API-03 · Pool C · PUT /api/products/:id
Raw: `reports/newman/23127178_api-03-product-update_ci-32580345226.json`
